...
export class AreasSelect {
  ...
  @Input() lp = 0;  // 显示省份数组的index，默认从第一个开始
  @Input() lc = 0;  // 显示城市数组的index，默认从第一个开始
  @Input() lr = 0;  // 显示地区数组的index，默认从第一个开始
  ...
  open(data) {
    ...
    this.generate();// 加载
    /*  let columns=this.picker.getColumns();
     let provclmn=columns[0];
      let cityclmn=columns[1];
      let regnclmn=columns[2];
      provclmn.selectedIndex=this.lp;
      cityclmn.selectedIndex=this.lc;
      regnclmn.selectedIndex=this.lr;*/
  ...
  private generate() {
    ...
    provinceIndex = provinceIndex === -1 ? this.lp : provinceIndex;
    provinceCol.selectedIndex = provinceIndex;
    this.provinceCol = provinceIndex;
    ...
    if (this.level >= 2) {
      ...
      cityIndex = cityIndex === -1 ? this.lc : cityIndex;
      this.cityCol = cityIndex;
    ...
    if (this.level === 3) {
      ...
      regionIndex = regionIndex === -1 ? this.lr : regionIndex;
      regionCol.selectedIndex = regionIndex;
      this.regionCol = regionIndex;
  ...
  private validate(picker) {
    ...
    //  provinceCol.selectedIndex=this.lp;
